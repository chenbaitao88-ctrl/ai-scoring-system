...
[database.py](../backend/database.py)用 SQLAlchemy/SQLite 保存队伍、作品和基础评分。这部分承接应用已有的列表、详情和综合评分导出。

新的任务流水线使用独立文件记录任务、评分尝试、结果快照、复核决定和最终锁定。每类事实有明确标识和关联，重试不覆盖成功尝试，人工采用可以指向具体快照。

这种组织方便核对过程，也留下了一个需要继续收尾的问题：旧版 Excel 读取综合评分表，权威结果服务读取尝试、采用和锁定记录，两者目前没有统一。结果表上的导出成功，不等于权威链路的全部条件都满足。
...
单用户、本机、单后端实例是当前运行范围。多用户权限、集中部署、统一批量权威导出和真实模型效果评估，还需要继续完成。
